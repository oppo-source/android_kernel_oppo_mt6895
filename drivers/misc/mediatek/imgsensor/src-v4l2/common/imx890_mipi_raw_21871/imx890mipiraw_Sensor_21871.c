...
static kal_int32 write_1st_eeprom_protect(struct subdrv_ctx *ctx, kal_uint16 enable)
{
    ...
    adaptor_i2c_wr_u8(ctx->i2c_client, IMX890_EEPROM_WRITE_ID_21871 >> 1, reg, 0x0);

    reg = 0x06ca;
    if (enable) {
        adaptor_i2c_wr_u8(ctx->i2c_client, IMX890_EEPROM_WRITE_ID_21871 >> 1, reg, 0x2);
    }
    else {
        adaptor_i2c_wr_u8(ctx->i2c_client, IMX890_EEPROM_WRITE_ID_21871 >> 1, reg, 0x0);
    }

    adaptor_i2c_rd_u8(ctx->i2c_client, IMX890_EEPROM_WRITE_ID_21871 >> 1, reg, &flag);
            ...
            ACDK_SENSOR_ENGMODE_STEREO_STRUCT * pStereodata)
{
    ...
    if(pStereodata != NULL) {
        ...
        if ((pStereodata->uSensorId == IMX890_SENSOR_ID_21871) && (data_length == CALI_DATA_MASTER_LENGTH)
            ...
            && (data_base == IMX890_AESYNC_START_ADDR_21871)) {
            ...
            LOG_INF("readback main aesync: %x %x %x %x %x %x %x %x\n",
                    read_cmos_eeprom_8(ctx, IMX890_AESYNC_START_ADDR_21871),
                    read_cmos_eeprom_8(ctx, IMX890_AESYNC_START_ADDR_21871+1),
                    read_cmos_eeprom_8(ctx, IMX890_AESYNC_START_ADDR_21871+2),
                    read_cmos_eeprom_8(ctx, IMX890_AESYNC_START_ADDR_21871+3),
                    read_cmos_eeprom_8(ctx, IMX890_AESYNC_START_ADDR_21871+4),
                    read_cmos_eeprom_8(ctx, IMX890_AESYNC_START_ADDR_21871+5),
                    read_cmos_eeprom_8(ctx, IMX890_AESYNC_START_ADDR_21871+6),
                    read_cmos_eeprom_8(ctx, IMX890_AESYNC_START_ADDR_21871+7));
