...
extern int pps_ovp_ctrl(int on);
int g_hw_version = 0;
...
/*svooc curv*/
/*0~50*/
struct batt_bcc_curves svooc_curves_soc0_2_50[BATT_BCC_ROW_MAX] = {0};
/*50~75*/
struct batt_bcc_curves svooc_curves_soc50_2_75[BATT_BCC_ROW_MAX] = {0};
/*75~85*/
struct batt_bcc_curves svooc_curves_soc75_2_85[BATT_BCC_ROW_MAX] = {0};
/*85~90*/
struct batt_bcc_curves svooc_curves_soc85_2_90[BATT_BCC_ROW_MAX] = {0};
		...
		for (j = 0; j < BATT_BCC_CURVE_MAX; j++) {
			...
			case BATT_SOC_0_TO_50:
				rc = of_property_read_u32_array(soc_node, strategy_temp[j],
									(u32 *)svooc_curves_soc0_2_50[j].batt_bcc_curve,
									length);
				svooc_curves_soc0_2_50[j].bcc_curv_num = length/4;
				break;
			case BATT_SOC_50_TO_75:
				rc = of_property_read_u32_array(soc_node, strategy_temp[j],
									(u32 *)svooc_curves_soc50_2_75[j].batt_bcc_curve,
									length);
				svooc_curves_soc50_2_75[j].bcc_curv_num = length/4;
				break;
			case BATT_SOC_75_TO_85:
				rc = of_property_read_u32_array(soc_node, strategy_temp[j],
									(u32 *)svooc_curves_soc75_2_85[j].batt_bcc_curve,
									length);
				svooc_curves_soc75_2_85[j].bcc_curv_num = length/4;
				break;
			case BATT_SOC_85_TO_90:
				rc = of_property_read_u32_array(soc_node, strategy_temp[j],
									(u32 *)svooc_curves_soc85_2_90[j].batt_bcc_curve,
									length);
				svooc_curves_soc85_2_90[j].bcc_curv_num = length/4;
				break;
			default:
				break;
			}
...
//	chip->vooc_gpio.pps_vbus_det_ctr1_gpio = of_get_named_gpio(node, "qcom,charging_pps-gpio", 0);
//	if (chip->vooc_gpio.pps_vbus_det_ctr1_gpio < 0) {
//		chg_err("chip->vooc_gpio.pps_vbus_det_ctr1_gpio not specified\n");
//	} else {
//		if (gpio_is_valid(chip->vooc_gpio.pps_vbus_det_ctr1_gpio)) {
//			rc = gpio_request(chip->vooc_gpio.pps_vbus_det_ctr1_gpio, "charging-pps-gpio");
//			if (rc) {
//				chg_err("unable to request gpio [%d]\n", chip->vooc_gpio.pps_vbus_det_ctr1_gpio);
//			} else {
//				gpio_direction_output(chip->vooc_gpio.pps_vbus_det_ctr1_gpio, 1);
//			}
//		}
//		chg_err("chip->vooc_gpio.pps_vbus_det_ctr1_gpio =%d\n", chip->vooc_gpio.pps_vbus_det_ctr1_gpio);
//	}
...
set_gpio_fail:
	...
	printk("oplus_pps_eint_unregister retval:%d\n",retval);
...
int oplus_vooc_choose_bcc_fastchg_curve(struct oplus_vooc_chip *chip)
{
	...
	if (batt_soc_plugin == FAST_SOC_0_TO_50) {
		...
		for (i = 0; i < BATT_BCC_ROW_MAX; i++) {
			svooc_curves_target_soc_curve[i].bcc_curv_num = svooc_curves_soc0_2_50[i].bcc_curv_num;
			memcpy((&svooc_curves_target_soc_curve[i])->batt_bcc_curve,
				(&svooc_curves_soc0_2_50[i])->batt_bcc_curve,
				sizeof(struct batt_bcc_curve) * (svooc_curves_soc0_2_50[i].bcc_curv_num));
		}
	} else if (batt_soc_plugin == FAST_SOC_50_TO_75) {
		chg_err("soc is 50~75!\n");
		for (i = 0; i < BATT_BCC_ROW_MAX; i++) {
			svooc_curves_target_soc_curve[i].bcc_curv_num = svooc_curves_soc50_2_75[i].bcc_curv_num;
			memcpy((&svooc_curves_target_soc_curve[i])->batt_bcc_curve,
				(&svooc_curves_soc50_2_75[i])->batt_bcc_curve,
				sizeof(struct batt_bcc_curve) * (svooc_curves_soc50_2_75[i].bcc_curv_num));
		}
	} else if (batt_soc_plugin == FAST_SOC_75_TO_85) {
		chg_err("soc is 75~85!\n");
		for (i = 0; i < BATT_BCC_ROW_MAX; i++) {
			svooc_curves_target_soc_curve[i].bcc_curv_num = svooc_curves_soc75_2_85[i].bcc_curv_num;
			memcpy((&svooc_curves_target_soc_curve[i])->batt_bcc_curve,
				(&svooc_curves_soc75_2_85[i])->batt_bcc_curve,
				sizeof(struct batt_bcc_curve) * (svooc_curves_soc75_2_85[i].bcc_curv_num));
		}
	} else if (batt_soc_plugin == FAST_SOC_85_TO_90) {
		chg_err("soc is 85~90!\n");
		for (i = 0; i < BATT_BCC_ROW_MAX; i++) {
			svooc_curves_target_soc_curve[i].bcc_curv_num = svooc_curves_soc85_2_90[i].bcc_curv_num;
			memcpy((&svooc_curves_target_soc_curve[i])->batt_bcc_curve,
				(&svooc_curves_soc85_2_90[i])->batt_bcc_curve,
				sizeof(struct batt_bcc_curve) * (svooc_curves_soc85_2_90[i].bcc_curv_num));
		}
	}
