...
static void oplus_comm_ui_soc_update(struct oplus_chg_comm *chip)
{
	...
	if (chip->vbat_min_mv <
		...
		vbat_uv_jiffies =
			chip->vbat_uv_jiffies + (unsigned long)(20 * HZ);
...
check_update_time:
	...
	chg_info("ui_soc=%d, real_soc=%d, update_delay=%u\n", chip->ui_soc,
		 chip->soc, jiffies_to_msecs(update_delay));
		 ...
		 struct panel_event_notification *notification, void *client_data)
{
	...
	chg_info("Notification type:%d, early_trigger:%d",
