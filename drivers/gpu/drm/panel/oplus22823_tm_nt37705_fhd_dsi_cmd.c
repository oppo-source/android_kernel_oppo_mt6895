	...
	//fhd_sdc_120_mode
{
	...
	.lcm_esd_check_table[0] = {
		.cmd = 0x0A, .count = 1, .para_list[0] = 0x9C, .mask_list[0] = 0x00,
	},
...
static int lcm_setbacklight_cmdq(void *dsi, dcs_write_gce cb, void *handle, unsigned int level)
{
	...
	if (level == 1) {
		pr_info("[LCM][INFO][%s:%d]enter aod\n", __func__, __LINE__);
		return 0;
	} else if (level > 4095) {
		level = 4095;
	}
	...
	void *minfps, struct drm_display_mode *m)
{
	...
	if (min_fps->minfps_flag == 0) {
		...
		ADFRFUNC("auto_off_minfps_cmd,ext_frame=%d\n", ext_frame);
