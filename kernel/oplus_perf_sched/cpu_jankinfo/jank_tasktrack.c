...
void update_tasktrack_time_win(void)
{
	...
	jank_systrace_print_idx(JANK_SYSTRACE_TASKTRACK, "debug0_tick", 0, 2);
	...
	for (idx = 0; idx < task_num; idx++) {
		winidx = trace_info->task_info[idx].winidx;
		winidx = winidx_add(winidx, 1);

		memset(&trace_info->task_info[idx].time[winidx],
					0, sizeof(struct state_time));

		nowtype = trace_info->task_info[idx].now_type;
		last_update_time = trace_info->task_info[idx].last_update_time;
		...
		jank_systrace_print_idx(JANK_SYSTRACE_TASKTRACK,
				"debug5_unprocessed_delta", idx, unprocessed_delta);

		for (type = TRACE_RUNNING; type < TRACE_CNT; type++) {
			jank_dbg("m0: type=%d, dbg_borrowed_time=%llu\n",
					type, trace_info->task_info[idx].borrowed_time[type]);
			...
			if (unprocessed_delta > JANK_WIN_SIZE_IN_NS) {
				if (is_current_type(type, nowtype)) {
					 ...
					 ****************************************************/
					if (trace_info->task_info[idx].delta_bak[type] == 0) {
						delta = 0;
					} else {
						delta = JANK_WIN_SIZE_IN_NS;
						trace_info->task_info[idx].borrowed_time[type] +=
							JANK_WIN_SIZE_IN_NS;
					}
					...
					delta = 0;
					trace_info->task_info[idx].borrowed_time[type] = 0;
				...
				/****************************************************
				 * FIXME:
				 *     how to deal with the first window
				 ****************************************************/
				delta = trace_info->task_info[idx].delta[type] -
							trace_info->task_info[idx].delta_bak[type];

				jank_systrace_print_idx(JANK_SYSTRACE_TASKTRACK,
							"debug6_delta", idx, delta);

				#ifdef JANK_DEBUG
				dbg_delta1 = delta;
				dbg_task_info_delta =
					trace_info->task_info[idx].delta[type];
...
#ifdef JANK_DEBUG
			dbg_delta_win_align_bak =
					trace_info->task_info[idx].delta_win_align[type];
#endif
			...
			trace_info->task_info[idx].time[winidx].val[type] = delta;
			jank_systrace_print_idx(JANK_SYSTRACE_TASKTRACK,
			...
			jank_dbg("m5: winidx=%d, type=%d, nowtype=%d, "
					...
					"\n",
					winidx, type, nowtype,
					dbg_unprocessed_delta, dbg_unprocessed_delta>>20,
					delta, delta>>20,
					dbg_delta1, dbg_task_info_delta,
					dbg_task_info_delta_bak,
					dbg_delta2, dbg_borrowed_time,
					(dbg_delta_win_align-dbg_delta_win_align_bak)>>20);
#endif
		}
		trace_info->task_info[idx].winidx = winidx;
		jank_dbg("m6: winidx=%d, dbg_delta=%llu(%llu)\n",
				winidx, dbg_delta >> JANK_NS_PER_MS_SHIFT, dbg_delta);
	}
}
			...
			unsigned long type)
{
	...
	unsigned long tmptype = BIT(type);
	...
	jank_dbg("cb: comm=%s, type=%d, nowtype=%d, r=%d\n",
			p->comm, type, nowtype, trace_info->task_info[idx].now_type);
#ifdef JANK_DEBUG
	jank_systrace_print(p->pid, "dbg999_nowtype", type);
#endif
}
			...
			unsigned long type, u64 time_in_ns)
{
	...
	if (!trace_info || !task || !is_task_traced(task, &idx) || !time_in_ns)
		return;

	nowtype = trace_info->task_info[idx].now_type;

	trace_info->task_info[idx].delta[type] += time_in_ns;
	if ((type == TRACE_SLEEPING) && is_inbinder(nowtype)) {
		trace_info->task_info[idx].delta[TRACE_SLEEPING_INBINDER] +=
			time_in_ns;
	} else if ((type == TRACE_SLEEPING) && is_infutex(nowtype)) {
		trace_info->task_info[idx].delta[TRACE_SLEEPING_INFUTEX] +=
			time_in_ns;
	}
	trace_info->task_info[idx].last_update_time =
...
static int proc_task_track_read(struct seq_file *m, void *v)
{
	...
	ti = &trace_info_snap->task_info[0];
	...
	for (i = 0; i < TASK_TRACK_NUM; i++) {
		...
		last_update_time = ti[i].last_update_time;
		...
		for (j = 0; j < JANK_WIN_CNT; j++) {
			...
			for (k = 0; k < TRACE_CNT; k++) {
				if (j == 0) {
					...
					time = ti[i].time[idx].val[k];
				}
				#ifdef JANK_DEBUG
				dbg_total += time;
				#endif
				seq_printf(m, "%llu%s", (time >> JANK_NS_PER_MS_SHIFT),
							(k == TRACE_CNT-1) ? "" : " ");
				...
				jank_dbg("j1=%d, idx=%d, winidx=%d, type=%d, nowtype=%d, "
					...
					unprocessed_delta>>20, time>>20,
					dbg_task_info_delta, dbg_task_info_delta_win_align);
			}
			#ifdef JANK_DEBUG
			seq_printf(m, "[%llu, %d]\n",
				dbg_total >> JANK_NS_PER_MS_SHIFT,
				(dbg_total/1024/1024)-128);

			jank_dbg("j2=%d, idx=%d, winidx=%d, "
					"unprocessed_delta=%llu, dbg_total=%llu\n",
					j, idx, winidx,
					unprocessed_delta>>20, dbg_total>>20);
