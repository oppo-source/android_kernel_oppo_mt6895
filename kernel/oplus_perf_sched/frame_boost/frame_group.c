...
static void add_task_to_frame_group(struct frame_group *grp, struct task_struct *task)
{
	...
	/* Staic frame task's depth is zero */
	ots->fbg_depth = 0;
}
...
void set_ui_thread(int pid, int tid)
{
	...
	if (grp->ui)
		remove_task_from_frame_group(grp->ui);
...
static void update_frame_zone(struct frame_group *grp, u64 wallclock)
{
	...
	if (unlikely(sysctl_frame_boost_debug))
		trace_printk("update frame zone=%d\n", grp->frame_zone);
}
...
static struct oplus_sched_cluster *best_cluster(struct frame_group *grp)
{
	...
	if (grp == &default_frame_boost_group)
		boosted_grp_util += schedtune_grp_margin(util, stune_boost[BOOST_MIGR]);
	...
	unsigned long *util)
{
	...
	if (unlikely(!sysctl_frame_boost_enable))
		return false;
	...
	if (valid_freq_querys(query_cpus, grp)) {
		...
		boosted_policy_util = policy_util +
			schedtune_grp_margin(policy_util, stune_boost[BOOST_FREQ]);
	}
...
unlock_fbg:
	...
	*util = max(raw_util, boosted_policy_util);
	...
	if (valid_freq_querys(query_cpus, grp)) {
		policy_util = grp->policy_util;
		boosted_policy_util = policy_util +
			schedtune_grp_margin(policy_util, rt_stune_boost[BOOST_FREQ]);
	}
...
unlock:
	...
	if (need_update_prev_freq) {
		rq = cpu_rq(prev_cpu);
		cpufreq_update_util(rq, SCHED_CPUFREQ_DEF_FRAMEBOOST);
	}
...
unlock:
	...
	if (need_update) {
		rq = task_rq(tsk);
		cpufreq_update_util(rq, SCHED_CPUFREQ_SF_FRAMEBOOST);
	}
...
retry:
	...
	if (ret == false && grp->available_cluster && use_avail_cls) {
...
bool fbg_rt_task_fits_capacity(struct task_struct *tsk, int cpu)
{
	...
	grp_util = raw_util + schedtune_grp_margin(raw_util, rt_stune_boost[BOOST_MIGR]);
